...
import type { AccountTreeDataProvider } from "../providers/account/account-tree-data-provider";
import type { BuildTreeDataProvider } from "../providers/build/build-tree-data-provider";
import type { PipelineTreeDataProvider } from "../providers/pipeline/pipeline-tree-data-provider";
import type { StageTreeDataProvider } from "../providers/stage/stage-tree-data-provider";
import type { AccountManager } from "../services/account-manager";
import type { AccountData } from "../types/account";
import type { Build } from "../types/builds";
import type { TimelineRecord } from "../types/stages";
import type { Pipeline, Project, TemplateParameters } from "../types/types";
...
/**
 * Normalize Azure Pipelines log line:
 * - Remove timestamp prefix (first 29 chars)
 * - Strip Azure formatting markers like ##[command], ##[section], ##[debug], ##[warning], ##[error], ##[group], ##[endgroup]
 */
const cleanLogLine = (line: string): string => {
	// Defensive: handle short lines
	const sliced = line.length >= 29 ? line.slice(29) : line;

	// Strip ANSI color/style sequences (e.g., \x1b[32m, [33m, \x1b[0m)
	const ANSI_SGR_REGEX = /\u001b\[[0-9;]*m/g;
	let cleaned = sliced.replace(ANSI_SGR_REGEX, "");

	// Strip command/group/section markers (with or without leading ##)
	cleaned = cleaned
		.replace(/^((##)?\[command\])\s*/i, "")
		.replace(/^((##)?\[section\])\s*/i, "")
		.replace(/^((##)?\[group\])\s*/i, "")
		.replace(/^((##)?\[endgroup\])\s*/i, "");

	// Replace warning/error tags to keep visual cue without raw tag text
	cleaned = cleaned
		.replace(/^((##)?\[warning\])\s*/i, "⚠ ")
		.replace(/^((##)?\[error\])\s*/i, "✖ ");
			...
			async () => {
				try {
					...
					const newAccount = await vscode.window.withProgress(
						...
						async () => {
							return await accountManager.addAccount(
								organization.trim(),
								pat.trim(),
								note?.trim() || "",
							);
			...
			async () => {
				try {
					...
					if (projects.length === 0) {
						vscode.window.showInformationMessage(
							"No projects available to filter.",
						);
						return;
					}
					...
					const quickPickItems = projects
						...
						.map((project) => ({
							label: project.name,
							picked:
								currentFilter.size === 0 || currentFilter.has(project.name),
							project,
						}));
					...
					if (selectedItems.length === projects.length) {
						...
						vscode.window.showInformationMessage(
							"Filter cleared - showing all projects.",
						);
					} else {
						// Apply filter
						const selectedProjectNames = selectedItems.map(
							(item) => item.label,
						);
						...
						vscode.window.showInformationMessage(
							`Filter applied - showing ${projectsText}.`,
						);
					}
				} catch (error) {
					const message =
						error instanceof Error ? error.message : "Unknown error";
					vscode.window.showErrorMessage(
						`Failed to filter pipelines: ${message}`,
					);
				}
			},
		),
		...
		vscode.commands.registerCommand(
			...
			async ({ timelineRecord }: { timelineRecord: TimelineRecord }) => {
				try {
					...
					const formattedLog = logContent.value.map((line) =>
						cleanLogLine(line),
					);
		...
		vscode.commands.registerCommand(
			...
			async ({ timelineRecord }: { timelineRecord: TimelineRecord }) => {
				try {
					...
					const { handleStageRetry } = await import(
						"../services/stage-retry-service.js"
					);
		...
		vscode.commands.registerCommand(
			...
			async ({ timelineRecord }: { timelineRecord: TimelineRecord }) => {
				try {
					...
					const actionText =
						decision.value === "approve" ? "Approving" : "Rejecting";
			...
			async () => {
				try {
					...
					if (pipelineDefinition?.configuration) {
						...
						console.log(
							"Pipeline configuration found - YAML path:",
							yamlPath,
							"Repo ID:",
							repoId,
						);
						...
						if (yamlPath && repoId) {
							...
							if (yamlContent) {
								console.log(
									"YAML file fetched successfully, length:",
									yamlContent.length,
								);
								console.log("YAML path:", yamlPath);
								console.log(
									"YAML content preview:",
									yamlContent.substring(0, 500),
								);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async () => {
				...
				vscode.window.showInformationMessage(
					`Azure Pipelines polling ${status}`,
				);
			},
		),
	);
