...
export async function getProjects(): Promise<Project[]> {
    try {
        ...
        vscode.window.showErrorMessage('Error listing projects. Please check if your PAT has the correct permissions.');
        return [];
    }
}
...
export async function getPipelines(project: string): Promise<Pipeline[]> {
    try {
        ...
        vscode.window.showErrorMessage('Error listing pipelines. Check if your PAT has the correct permissions.');
        return [];
    }
}

export async function getBuildsByDefinitionId(project: string, definitionId: number): Promise<Build[]> {
    try {
        const { pat, organization } = await getConfiguration();
        ...
        vscode.window.showErrorMessage('Error listing builds. Please check if your PAT has the correct permissions.');
        return [];
    }
}


export async function getCommitMessage(project: string, repositoryId: string, commitId: string): Promise<string> {
    try {
        const { pat, organization } = await getConfiguration();
        ...
        vscode.window.showErrorMessage('Error getting commit message. Please check if your PAT has the correct permissions.');        
        return '';
    }
}
...
export async function getBuildStages(project: string, buildId: number): Promise<BuildTimeline> {
    try {
        ...
        vscode.window.showErrorMessage('Error getting build stages. Please check if your PAT has the correct permissions.');
        return { records: [] };
    }
}